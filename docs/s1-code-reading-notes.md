...
### 任务 4 目验收口弧(run45-50) + S1 真收口
- **窗口事件(run45-48)**: 用户注视对象是冻结的幽灵窗口(无 HUD、两帧逐像素相同)→
  "没有出现"误判; 其后用户飞越墙(x=-568)到其背面/脸侧 → "糊在镜头上的薄片/奇妙三角形"
  = 零厚度膜被贴脸与侧向视角所看(非 bug)。
- **run47 根因修复(探针 NDC 绑定)**: 任务 4 接线时探针(PIPELINE_PROBE, NDC 钉死屏幕的
  数据板, RGB=矩阵真值编码)沿用主管线流式 VBO/IBO → 墙的世界坐标被 NDC 探针当屏幕坐标 →
  糊在镜头上闪烁的薄片/满屏纯色块(run44 红|蓝整块、run45"太阳上方小三角"均其钉位)。
  修复 = 探针换回自有静态 S0 缓冲(vertexBuffer.slice()/indexBuffer, firstIndex=0)。
  同期墙 -400 → -100 拉近目验(一行常量, 超远规格留 S2)。
- **run49 A/B 因子裁决(新增 DHVK_NOWALL/DHVK_NOPROBE 环境因子开关, 启动日志记录)**:
  DHVK_NOPROBE=1 → 满屏纯色块消失, 画面干净 → 纯色块 = 探针数据板的正身岗位(屏幕钉死、
  颜色=数据编码、非贴图); 屏上留存的彩虹光柱 = 墙本尊(世界坐标、立在地形上、共享深度)。
- **run50 目验裁决**: 用户报"**看到流动了**" —— 墙逐帧重合成波(±1.5 块轮廓起伏 + 色相
  流动)肉眼确认 = 任务 4 验收物出现。"最上面好奇怪不是直的"裁决: 顶边按设计即非直线
  (顶排顶点随波 ±1.5 块起伏), 侧向掠视角把它拉伸成锯齿; 400 块外(≈0.2°)自动平滑为
  "微起伏近平直" —— 非缺陷, 是其呼吸的形态。
- **预算闸门全程记录(run44-50)**: 稳态 350-500µs(1ms 闸门富余 2-4 倍); 各 run 初帧/
  前几十帧 1.5-7ms 孤立尖峰 = 官方瞬态环首块 512KB 申请的一次性成本 + 偶发渲染线程颠簸,
  闸门按设计检测并报告(不兜底)。
- **S1 真收口**: 机制门槛(正/负/堆 VVL 双零/VRS/合成 ring 流式)+ 目验(流动确认 + A/B
  归属)全部闭合。遗留 S2 验收位: 墙回超远 -400 规格(一行常量)做正面远观 + 遮挡 + 雾带
  三查; DHVK_NOWALL/DHVK_NOPROBE 因子开关保留供 S2 A/B 使用。

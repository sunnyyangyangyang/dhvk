...
**状态：✅ 任务 1 闭合**（步骤 12 目验三项目全过 + 步骤 13 R-c 定死 + 最终代码形态 VVL 双零：标准 run65/66 + 全开 run67 + 步骤 15 高度场退役独立 commit）
...
## 步骤 15：高度场退役（S1 合成 ring 退出生产路径）

- **删除**：`DHVK_MESH` 因子 + `DhVkClient.meshMode()` + `MESH_MODE` 分支 —— 帧路径恒走贪心带
  （IBO = u32 顺序 / IndexType.INT；合成环的 u16 分支随模式一并退场）。
- **保留**：`VoxelWallSynthesizer` 合成调试环（墙 A/B + 探针板，DHVK_NOWALL/NOPROBE 因子照旧）降级为
  机制验收工具（规格 §0：与高度场退役无关）；再验证入口 = `DHVK_SYNTRING=1`。
- 目验基线：run45-50（S1 归档截图）+ 本 spike run53-67；"高度场墙"外观自此在代码中退场。
...
- [x] 步骤 15：高度场（S1 合成 ring）退役 —— `DHVK_SYNTRING` 接管合成调试环入口，独立 commit + VVL 标准烟测双零

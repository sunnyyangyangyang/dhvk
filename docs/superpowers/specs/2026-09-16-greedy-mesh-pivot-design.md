...
- 日期：2026-09-16
- 状态：设计对话完成，用户全节批准（2026-09-16，全节批准 + 追加裁决）
- 取代 / 重定义：
...
| 2 | **LOD 阶梯（体素版）**：级联投票（§4.2）+ 逐级贪心 + 档位带宽表（带宽不变）+ per-cell UBO 并入堆（S2 v1 §3.3 原样：CHUNK_SECTION 成堆内描述符、官方动态通道退役为 push 壳、数据源判别探针） | 价目表出现 L0~L3 分行且数值合理；L0/L1 交界无 pop-in（近带切片 + 合成环目验）；VVL 双零；+1ms（RD32 / 5090）；UBO 数据源 = 堆（探针判别式） |
...
- **步骤2**：L1+ 网格派生成本实测（投票 + 贪心，§4.2）；visibility 双通道（S2 v1 §3.3）。
...
- **下一步**：用户审阅本规格 → 通过 → 调 writing-plans 技能出实施计划（任务切片 = §8 步骤序）。
